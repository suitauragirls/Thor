...
export const AdminOrders: React.FC = () => {
  ...
  return (
    <div id="admin-orders-page" className="space-y-6">
      ...
      {/* Filter & Search */}
      <div className="bg-[#FDFBF7] p-4 rounded-2xl border border-[#B8935A]/25 shadow-xs grid grid-cols-1 sm:grid-cols-12 gap-3">
        <div className="sm:col-span-8 relative">
          <input
            type="text"
            placeholder="Search by order ID (SBA-...), customer name, email, or mobile..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-4 py-2 border border-[#B8935A]/25 bg-[#FAF5EB]/50 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-[#3D0F1F] text-gray-900"
          />
        ...
        <div className="sm:col-span-4">
          <select
            ...
            className="w-full px-3 py-2 border border-[#B8935A]/25 rounded-xl text-xs bg-[#FDFBF7] text-gray-700 focus:outline-none focus:ring-1 focus:ring-[#3D0F1F]"
          >
      ...
      <div className="bg-[#FDFBF7] rounded-2xl border border-[#B8935A]/25 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-[#FAF5EB] border-b border-[#B8935A]/20 text-gray-600 font-bold uppercase tracking-wider">
              ...
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order) => (
                  <tr 
                    ...
                    className="hover:bg-[#FAF5EB]/50 transition cursor-pointer"
                    onClick={() => openAdminOrderDetails(order.orderNumber)}
                  >
                    
                    {/* 1. Order ID */}
                    <td className="py-3 px-4 font-mono font-bold text-[#3D0F1F]">
                    ...
                    <td className="py-3 px-4" onClick={(e) => e.stopPropagation()}>
                      {order.paymentStatus === 'Paid' ? (
                        ...
                        <div className="space-y-1">
                          <span className="px-2.5 py-1 bg-amber-50 text-amber-900 border border-amber-300 rounded-full font-bold text-[10px] inline-flex items-center gap-1">
                    ...
                    <td className="py-3 px-4" onClick={(e) => e.stopPropagation()}>
                      <select
                        ...
                        className={`text-xs font-bold border rounded-lg px-2.5 py-1 transition cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#3D0F1F] ${getStatusBadge(order.status)}`}
                      >
                        {ORDER_STATUSES.map((st) => (
                          <option key={st} value={st}>{st}</option>
                        ))}
                    ...
                    <td className="py-3 px-4 text-right" onClick={(e) => e.stopPropagation()}>
                      <button
                        onClick={() => openAdminOrderDetails(order.orderNumber)}
                        className="px-3 py-1.5 bg-[#B8935A]/10 hover:bg-[#3D0F1F] hover:text-[#FAF5EB] text-[#3D0F1F] border border-[#B8935A]/25 rounded-lg text-xs font-semibold inline-flex items-center gap-1 transition cursor-pointer"
