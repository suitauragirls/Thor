...
export const DealOfTheDaySection: React.FC = () => {
  ...
  const targetProductId = dealOfTheDay?.productId || (products[0]?.id || '');
  ...
  return (
    <section 
      ...
      className="py-3 sm:py-5 bg-[#F1E8DF] border-b border-[#9A6A3A]/25 relative overflow-hidden"
    >
      <div className="max-w-4xl mx-auto px-3 sm:px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-4 sm:mb-6 gap-3 sm:gap-4">
          ...
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#241D1B] text-[#211C1A] text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] mb-2.5 border border-[#9A6A3A]/40 shadow-xs">
            ...
            <p className="text-xs sm:text-sm text-[#211C1A]/80 mt-1.5 max-w-xl font-normal leading-relaxed">
          ...
          <div className="inline-flex items-center gap-2 bg-[#241D1B] text-[#211C1A] px-3 py-1.5 rounded-xl border border-[#9A6A3A]/50 shadow-md shrink-0 self-start md:self-end">
            <div className="flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-black animate-spin" style={{ animationDuration: '8s' }} />
            ...
            <div className="flex items-center gap-1 font-mono text-xs">
              <span className="bg-[#1F0710] border border-[#9A6A3A]/40 text-black font-black px-1.5 py-0.5 rounded text-[11px]">
                {String(timeLeft.hours).padStart(2, '0')}h
              </span>
              <span className="text-black font-bold text-[10px] animate-pulse">:</span>
              <span className="bg-[#1F0710] border border-[#9A6A3A]/40 text-black font-black px-1.5 py-0.5 rounded text-[11px]">
        ...
        {/* ============================================================ */}
        <div className="bg-white rounded-2xl sm:rounded-3xl border-2 border-[#9A6A3A]/40 shadow-xl overflow-hidden max-w-3xl mx-auto">
          
          {/* Top Dark Banner Header (Matching Hero Slide Style) */}
          <div className="bg-[#241D1B] text-[#211C1A] px-3.5 sm:px-5 py-2 flex items-center justify-between border-b border-[#9A6A3A]/35">
            <div className="flex items-center gap-2 min-w-0 pr-2">
              <Crown className="w-3.5 h-3.5 text-black shrink-0 animate-pulse" />
              <span className="font-serif font-bold text-[11px] sm:text-xs tracking-wider uppercase text-black truncate">
                {dealOfTheDay?.badgeText || `DEAL OF THE DAY - ${product.name}`}
              </span>
            </div>
            
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-[#F1E8DF]/10 border border-[#C7A77A]/40 text-black text-[8px] sm:text-[9px] font-black uppercase tracking-widest shrink-0">
          ...
          <div className="p-2.5 sm:p-4 flex flex-row items-center gap-3 sm:gap-4">
            ...
            <div className="w-[110px] sm:w-[150px] md:w-[170px] shrink-0 relative">
              <div 
                ...
                className="relative aspect-[3/4] sm:aspect-[4/5] rounded-xl overflow-hidden border border-[#9A6A3A]/30 cursor-pointer group bg-[#F1E8DF] shadow-2xs p-0.5 flex items-center justify-center"
              >
                <img 
                  src={heroImage} 
                  alt={product.name} 
                ...
                <button
                  ...
                  className={`absolute top-1.5 right-1.5 p-1.5 rounded-full border transition cursor-pointer shadow-xs ${
                    isWishlisted 
                      ...
                      : 'bg-white/90 border-[#9A6A3A]/40 text-gray-700 hover:text-[#211C1A] backdrop-blur-xs'
                  }`}
            ...
            <div className="flex-1 min-w-0 space-y-1.5 sm:space-y-2 flex flex-col justify-between">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center justify-between gap-1.5">
                  <span className="inline-block px-2 py-0.5 rounded-md bg-[#F1E8DF] text-[#211C1A] font-mono font-bold text-[9px] sm:text-[10px] uppercase tracking-wider border border-[#9A6A3A]/30 truncate">
                    {product.category || 'KURTIS'}
                  </span>

                  <div className="flex items-center gap-0.5 text-black text-[10px] sm:text-xs font-bold shrink-0">
                ...
                <h3 
                  onClick={() => navigateToProduct(product.id)}
                  className="font-serif text-xs sm:text-base font-bold text-[#211C1A] leading-tight line-clamp-2 hover:text-black transition cursor-pointer"
                >
                  {product.name}
                </h3>
                ...
                <div className="flex items-baseline gap-1.5 flex-wrap pt-0.5">
                  <span className="font-serif text-base sm:text-xl font-black text-[#211C1A]">
                  ...
                  <span className="px-1.5 py-0.5 rounded bg-[#241D1B] text-[#211C1A] font-extrabold text-[9px] sm:text-[10px] border border-[#9A6A3A]/40">
                    {discountPercent}% OFF
                  </span>
                </div>

                {/* Stock Urgency Notice */}
                <div className="text-[10px] sm:text-xs font-bold text-black flex items-center gap-1 pt-0.5">
                  <Flame className="w-3 h-3 text-black shrink-0" />
              ...
              <div className="grid grid-cols-2 gap-1.5 pt-1.5 border-t border-gray-100">
                <button
                  ...
                  className="py-1.5 sm:py-2 px-2 bg-[#F1E8DF] hover:bg-[#F2E8D5] text-[#211C1A] font-serif font-bold text-[10px] sm:text-xs uppercase tracking-wider rounded-lg border border-[#9A6A3A]/50 flex items-center justify-center gap-1 shadow-2xs transition active:scale-95 cursor-pointer"
                >
                  {isAddedToBag ? <Check className="w-3 h-3 text-emerald-600" /> : <ShoppingBag className="w-3 h-3 text-black" />}
                ...
                <button
                  ...
                  className="py-1.5 sm:py-2 px-2 bg-[#241D1B] hover:bg-[#20050E] text-[#211C1A] font-serif font-bold text-[10px] sm:text-xs uppercase tracking-wider rounded-lg border border-[#9A6A3A] flex items-center justify-center gap-1 shadow-xs transition active:scale-95 cursor-pointer"
                >
